...
import { BaseSystem, ItemInfo, NumberPool, SpatialHashingInterest } from 'ecs-threejs';
...
export class BaseRoom extends BaseSystem {
	...
	constructor(app: ServerApp, listPacker: MessagesPackerList) {
		...
		this.interestUpdate = new SpatialHashingInterest(this.interesetDistance, app.config.worldWrap, app.config.worldSize);
	}
	...
	sendToConnection(user: ConnectionData, idMessage: IdsMessage, message: protocol.IMessage) {
		if (user && user.socket)
	...
	setToUid(idUser: number, idMessage: IdsMessage, message: protocol.IMessage) {
		...
		return this.sendToConnection(user, idMessage, message);
	}
	...
	getWorldState(idUser: number) {
				...
				if (userEntity) {
					...
					for (let entity of visibleSet) {
						...
						var state: protocol.IEntityInfo = {
							id: Number(entity.idEntity),
							position: entity.getPosition(),
							velocity: entity.getVelocity(),
							angle: entity.getRotationDeg()
						};
		...
		//console.log("SendTo:",idUser, 'count:', list.length);
		return list;
	}
	...
	onSocketUpdate() {
		...
		if (this.interestActive) {
			for (var idUser in this.connectedUsers) {
				...
				var timePack = this.makeMessage(protocol.IdMessages.IScTimestamp, msgTimestamp);
				var statePack = this.makeMessage(protocol.IdMessages.IScWorldStateUpdate, msgWorldState);
				var buffer = mergeBuffer(timePack, statePack)
				this.sendSocketBuffer(connection.socket, buffer);
			}
		}
		else {
			this.InsertFirstPack(protocol.IdMessages.IScTimestamp, msgTimestamp);
			const msgWorldState: protocol.IScWorldStateUpdate = { list: this.getWorldState(-1) };
			this.addPack(protocol.IdMessages.IScWorldStateUpdate, msgWorldState);
		}
		// а ведь в буффере могли быть какие-то данные, надо их отправить
		this.sendFullBuffer();
	}
